...
import { View, Text, Dimensions, TouchableOpacity, Button, Alert, Image, NativeScrollEvent } from 'react-native';
...
const Videos = () => {
    ...
    const [playVideoID, setPlayVideoID] = useState<string | undefined>("sowGYbxTPgU");
    ...
    return (
        <ScrollScreenContainer
            props={{
                ...
                snapToInterval: height * 0.6 + HEADER_HEIGHT,
        ...
        >
            <View style={tw`flex flex-row justify-between w-full h-${HEADER_HEIGHT}px`}>
            ...
            {
                films.map(film => (
                    <View key={film.title} style={tw`flex-col h-${height * 0.6}px shadow-md bg-white rounded-xl w-${width}px self-center px-2 my-5 items-center ${loadingVideos ? 'hidden' : ''}`}>
                        ...
                        <YoutubePlayer
                            ...
                            onReady={() => setLoadingVideos(false)}
                        />

                        <View style={tw`self-start mt-2`}>
                            <Text style={tw`text-3xl font-bold `}>{film.title}</Text>
                            <Text style={tw`text-lg text-gray-500 mt-2`}>{film.year}</Text>
                            <Text style={tw`text-sm text-gray-600 mt-2`}>{film.desc}
                                    <TouchableOpacity>
